...
all: $(BUILD_DIR)/fancord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/fancord_rc.o: fancord.rc assets/fancord.ico
	mkdir -p $(BUILD_DIR)
	windres fancord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/fancord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/fancord.desktop
	rm -f $(PREFIX)/share/pixmaps/fancord.png
	rm -f $(PREFIX)/share/pixmaps/fancord.svg
	rm -f $(PREFIX)/bin/fancord

install: uninstall
	cp fancord.desktop $(PREFIX)/share/applications/fancord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/fancord.svg
	cp $(BUILD_DIR)/fancord $(PREFIX)/bin/fancord

run: all
	cp assets/fancord.gschema.xml $(BUILD_DIR)/assets/fancord.gschema.xml
