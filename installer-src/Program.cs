...
namespace FancordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private FancordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Fancord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("FancordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "FancordInstaller_WebView2");
            ...
            )
            {
                var result = MessageBox.Show(
                    "The Fancord Installer needs a small component (WebView2) to run.\n\nClick OK to download and install it automatically.",
                    "One-time setup required",
                    MessageBoxButtons.OKCancel,
                    MessageBoxIcon.Information
                );
                if (result != DialogResult.OK) { Application.Exit(); return; }
                try
                {
                    var bootstrapper = Path.Combine(Path.GetTempPath(), "MicrosoftEdgeWebview2Setup.exe");
                    using (var client = new System.Net.Http.HttpClient())
                    {
                        client.DefaultRequestHeaders.Add("User-Agent", "Fancord-Installer/2.0");
                ...
                {
                    MessageBox.Show(
                        "Automatic install failed. Please install WebView2 manually from:\nhttps://developer.microsoft.com/en-us/microsoft-edge/webview2/",
                        "Setup Failed",
            ...
            _backend = new FancordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("FancordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("FancordInstaller.icon.ico"))
                    {
    ...
    [ComVisible(true)]
    public class FancordBackend
    {
        private LauncherForm _form;
        private WebView2 _webView;
        private HttpClient _http;
        private string _distDir;
        private string _extractRootDir;
        private string _exeDir;

        const string GITHUB_REPO = "saintmanuser/fancord";
        const string DIST_ZIP = "fancord-dist.zip";

        public FancordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _extractRootDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Fancord", "extracted");
            _distDir = Path.Combine(_extractRootDir, "dist", "desktop");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"fancord\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Fancord is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Fancord → not a third-party mod
                if (content.Contains("\"fancord\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "fancord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Fancord...", readMB, totalMB);
                        }
                    }
                }
            }
            ...
            await Task.Run(() => 
            {
                if (Directory.Exists(_extractRootDir)) Directory.Delete(_extractRootDir, true);
                Directory.CreateDirectory(_extractRootDir);

                // Normalize _extractRootDir with a guaranteed trailing separator for safe StartsWith checks
                var normalizedRootDir = _extractRootDir.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
                ...
                using (var archive = ZipFile.OpenRead(tmpZip))
                {
                    ...
                    foreach (var entry in archive.Entries)
                    {
                        ...
                        var fullPath = Path.GetFullPath(Path.Combine(_extractRootDir, entryPath));

                        // Security check: ensure resolved path stays inside _extractRootDir
                        if (!fullPath.StartsWith(normalizedRootDir, StringComparison.OrdinalIgnoreCase))
                            continue;
        ...
        private void DoInject(string resPath)
        {
            ...
            {
                // Déjà Fancord → on réinjecte proprement sans bail-out partiel
            ...
            SetProgress(92, "Configuring Fancord loader...");
            ...
            // Créer le backup Fancord si app.asar existe et qu'on n'a pas encore de backup
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"[Fancord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"fancord\""))
                {
            ...
            // 3. Clean up Fancord-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\Fancord\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"fancord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Fancord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
        ...
        private void CopyAssetsToDiscord(string resPath)
        {
            ...
            string[] dirsToCopy = { "mac", "multi-instance-icons", "modules" };
            ...
            var ghostServerSrc = Path.Combine(_extractRootDir, "ghost-server");
            if (Directory.Exists(ghostServerSrc)) CopyDirectory(ghostServerSrc, Path.Combine(appBase, "ghost-server"));
