...
try { _stealthActive = localStorage.getItem("Fancord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Fancord_stealthMode", "1") : localStorage.removeItem("Fancord_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, FancordAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#fancord-titlebar-btn",
    "#fancord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("fancord-stealth"); else document.body?.classList.remove("fancord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("fancord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("fancord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("fancord-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("fancord-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("fancord-stealth-change", listener);
        };
    }, []);
