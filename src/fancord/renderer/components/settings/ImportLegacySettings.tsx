...
import { BaseText } from "@Fancord/types/components";
import { React, useRef,useState } from "react";

import { cl } from "./Settings";

function detectSource(json: any): "equicord" | "vencord" | "fancord" | "unknown" {
    ...
    // Plugins spécifiques à Fancord
    if (pluginNames.some(p => ["FancordHelper", "equicordHelper"].includes(p))) return "fancord";
...
function cleanForFancord(json: any): any {
    ...
    // Supprimer les plugins propres à Equicord/Vencord qui n'existent pas dans Fancord
    ...
    async function processFile(file: File) {
        ...
        try {
            ...
            const sourceLabel =
                ...
                source === "fancord" ? "Fancord" : "inconnu";

            const cleaned = cleanForFancord(json);
            ...
            setStatus("success");
            setMessage(`✅ Settings ${sourceLabel} importés avec succès ! Redémarre Fancord pour appliquer.`);
        } catch (err: any) {
            setStatus("error");
    ...
    return (
        <div className={cl("category")}>
            ...
            <BaseText size="sm" style={{ marginBottom: "12px", opacity: 0.7 }}>
                Drag & drop your Equicord or Vencord backup JSON file here to import your settings into Fancord.
                Plugin-specific settings that don't exist in Fancord will be automatically removed.
            </BaseText>
