...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Fancord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Fancord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.FancordDir) {
        console.error("Cannot repair: using custom Fancord directory. Remove it in settings first.");
        process.exit(1);
    }
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Fancord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Fancord v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Fancord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("Fancord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("Fancord is already running. Quitting...");
            app.exit(0);
        }
    }
