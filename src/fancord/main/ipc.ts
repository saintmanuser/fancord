...
handleSync(IpcEvents.GET_GIT_HASH, () => Fancord_GIT_HASH);
...
// Handler pour VencordNative.fancord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.FancordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { FancordDir } = State.store;
    if (!FancordDir) return;

    const stats = await stat(FancordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(FancordDir);
});
