...
import { t, useTranslation } from "../autoTranslateFancord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("fancord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("fancord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Fancord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `fancord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("fancord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("fancord-event-logs");
