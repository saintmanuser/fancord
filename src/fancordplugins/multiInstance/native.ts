...
import { registerMediaPermissionsForSession } from "../../fancord/main/mediaPermissions";
...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "fancord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Fancord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__fancord_token', { value: TOKEN, writable: false });
        ...
        console.log("[FancordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[FancordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `fancord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:fancord-mi-${userId}`;
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Fancord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[FancordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__FANCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Fancord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.fancord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[FancordMI] arrangeSplit error:", e);
    }
}
