...
const GIFTING_BADGE_ICONS = [
    "https://i.ibb.co/hJ94PLM2/patron.png", // Patron (1x)
    "https://i.ibb.co/N6dnXJfX/luminary.png", // Luminary (3x)
    "https://i.ibb.co/Bm4M61P/Champion.png", // Champion (2x)
    "https://i.ibb.co/4khSb3v/icon.png", // Icon (6x)
    "https://i.ibb.co/99scj6xN/hero.png", // Hero (10x)
    "https://i.ibb.co/4wXq7mGQ/download.png", // Legend (20x)
];
const GIFTING_BADGE_LABELS = [
    t("Gifting Patron"),
    t("Gifting Champion"),
    t("Gifting Luminary"),
    t("Gifting Icon"),
...
const GIFTING_BADGE_IDS = ["gift_patron","gift_champion","gift_luminary","gift_icon","gift_hero","gift_legend"];
...
const BOOST_LABELS_RAW = ["1 Mois","2 Mois","3 Mois","6 Mois","9 Mois","12 Mois","15 Mois","18 Mois","24 Mois"];
...
const DISPLAY_NAME_FONTS = [
    { fontId: 0, label: "gg sans",  family: "'gg sans', 'Noto Sans', sans-serif",                                         sample: "Gg", weight: 700, italic: false },
    { fontId: 1, label: "Tempo",    family: "'Times New Roman', 'Georgia', serif",                                         sample: "Gg", weight: 700, italic: true  },
    { fontId: 2, label: "Sakura",   family: "'Nunito', 'Varela Round', 'Quicksand', 'Trebuchet MS', sans-serif",           sample: "Gg", weight: 800, italic: false },
    { fontId: 3, label: "Jellybean",family: "'Palatino Linotype', 'Palatino', 'Book Antiqua', serif",                      sample: "Gg", weight: 700, italic: true  },
    { fontId: 4, label: "Modern",   family: "'Raleway', 'Century Gothic', 'Gill Sans', sans-serif",                        sample: "Gg", weight: 300, italic: false },
    { fontId: 5, label: "Medieval", family: "'UnifrakturMaguntia', 'MedievalSharp', 'Papyrus', fantasy",                   sample: "Gg", weight: 400, italic: false },
    { fontId: 6, label: "8Bit",     family: "'Press Start 2P', 'Courier New', monospace",                                  sample: "Gg", weight: 400, italic: false },
    { fontId: 7, label: "Vampyre",  family: "'Playfair Display', 'Didot', 'Georgia', serif",                               sample: "Gg", weight: 700, italic: false },
];
...
function getFontFamilyById(fontId: number): string { return DISPLAY_NAME_FONTS.find(f => f.fontId === fontId)?.family ?? "inherit"; }
...
interface CustomProfileData {
    ...
    serverTagText?: string; serverTagIconId?: string; giftingLevel?: number;
}
...
function countDataFields(d: CustomProfileData | undefined | null): number {
    ...
    for (const k of Object.keys(d)) {
        ...
        count += (k === "nameplateSkuId" || k === "profileEffectId" || k === "decorationAsset") ? 10 : 1;
    }
    return count;
}
function mergeProfileFields(a: CustomProfileData, b: CustomProfileData, preferred: "a" | "b"): CustomProfileData {
    ...
    for (const key of allKeys) {
        ...
        if (aHas && bHas)       (result as any)[key] = preferred === "a" ? av : bv;
        else if (aHas && !bHas) (result as any)[key] = av;
        else if (bHas && !aHas) (result as any)[key] = bv;
    }
    return result;
}
    ...
    try {
        ...
        if (rawAll) {
            ...
            if (!storedData || Object.keys(storedData).length === 0) {
                ...
                if (rawOld) { try { storedData = JSON.parse(rawOld); } catch { storedData = {}; } isEnabled = enOld === "1"; }
            }
            return;
        }
...
async function loadData() {
    try {
        const lsAllData: Record<string, CustomProfileData> = {};
        const lsAllEnabled: Record<string, boolean> = {};
        try {
            const rawAll = localStorage.getItem(LS_ALL_DATA);
            if (rawAll) {
                Object.assign(lsAllData, JSON.parse(rawAll));
                const rawEnabled = localStorage.getItem(LS_ALL_ENABLED);
                if (rawEnabled) Object.assign(lsAllEnabled, JSON.parse(rawEnabled));
            }
        } catch { }

        const dsAllData    = await DataStore.get(DS_ALL_DATA)    as Record<string, CustomProfileData> | null;
        const dsAllEnabled = await DataStore.get(DS_ALL_ENABLED) as Record<string, boolean> | null;

        if (dsAllData && typeof dsAllData === "object" && Object.keys(dsAllData).length > 0) {
            const merged: Record<string, CustomProfileData> = {};
            const allIds = new Set([...Object.keys(lsAllData), ...Object.keys(dsAllData)]);
            for (const id of allIds) {
                const lsProfile = lsAllData[id] ?? {};
                const dsProfile = dsAllData[id] ?? {};
...
function injectHideStyle() {
    ...
    style.textContent = `
        ...
        [class*='nameTag'] [class*='panelSubtitle'] { color: transparent !important; }
        [class*='accountProfilePopout'] [class*='avatarWrap'] img,
        [class*='accountProfilePopout'] [class*='avatarWrap'] svg { opacity: 0 !important; }
    `;
    const inject = () => { if (!document.head) { requestAnimationFrame(inject); return; } document.head.appendChild(style); };
    inject();
}
function removeHideStyle() { document.getElementById(HIDE_STYLE_ID)?.remove(); }
...
function buildEffectCss(effect: string, color: string, color2: string): string {
    ...
    switch (effect) {
        case "neon":
            return `color:${baseColor}!important;-webkit-text-fill-color:${baseColor}!important;background:none!important;-webkit-background-clip:unset!important;background-clip:unset!important;text-shadow:none!important;filter:;`;
        case "gradient":
            return `background:linear-gradient(90deg,${baseColor},${secondColor})!important;-webkit-background-clip:text!important;background-clip:text!important;-webkit-text-fill-color:transparent!important;color:transparent!important;text-shadow:none!important;filter:none!important;`;
        case "toon":
            return `color:${baseColor}!important;-webkit-text-fill-color:${baseColor}!important;font-weight:900!important;background:none!important;-webkit-background-clip:unset!important;background-clip:unset!important;text-shadow:none!important;filter:none!important;-webkit-text-stroke:2px #000!important;paint-order:stroke fill!important;`;
        case "pop":
            return `color:${baseColor}!important;-webkit-text-fill-color:${baseColor}!important;font-weight:800!important;background:none!important;-webkit-background-clip:unset!important;background-clip:unset!important;filter:none!important;text-shadow:1px 1px 0 #fff,2px 2px 0 rgba(255,255,255,.7),4px 4px 0 rgba(0,0,0,.25)!important;`;
        default:
            return color
                ? `color:${color}!important;-webkit-text-fill-color:${color}!important;background:none!important;-webkit-background-clip:unset!important;background-clip:unset!important;text-shadow:none!important;filter:none!important;`
                : `color:inherit!important;-webkit-text-fill-color:inherit!important;background:none!important;-webkit-background-clip:unset!important;background-clip:unset!important;text-shadow:none!important;filter:none!important;`;
    }
}
...
function applyInlineFontStyles(clear: boolean) {
    ...
    if (clear || !isEnabled || !storedData.displayNameFont) {
        elements.forEach(el => {
            ...
            ["font-family","font-weight","font-style","color","-webkit-text-fill-color","text-shadow",
            "background","-webkit-background-clip","background-clip","-webkit-text-stroke","filter","paint-order"]
                .forEach(p => h.style.removeProperty(p));
        });
        return;
    }
    ...
    elements.forEach(el => {
        ...
        ["background","-webkit-background-clip","background-clip","-webkit-text-stroke","text-shadow",
        "color","-webkit-text-fill-color","filter","paint-order"].forEach(p => h.style.removeProperty(p));
        ...
        switch (effect) {
            ...
            case "pop":
                h.style.setProperty("color", baseColor, "important");
                h.style.setProperty("-webkit-text-fill-color", baseColor, "important");
                h.style.setProperty("text-shadow","3px 3px 0 #000,-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,2px 2px 0 #000","important");
                break;
            default:
                if (color) {
...
function stampOwnNameElements() {
    ...
    while ((node = walker.nextNode())) {
        ...
        if (!text) continue;
        let matched = false;
        for (const name of myDisplayNames) { if (text === name) { matched = true; break; } }
        if (!matched) continue;
        const parent = node.parentElement;
        if (!parent || isUsernameOrDiscriminator(parent) || !isDisplayNameOnly(parent)) continue;
        if (!parent.hasAttribute("data-cp-me")) parent.setAttribute("data-cp-me", "1");
        const gp = parent.parentElement;
        if (gp && !isUsernameOrDiscriminator(gp) && isDisplayNameOnly(gp)) {
            const gpText = gp.textContent?.trim() ?? "";
            let gpMatch = false;
            for (const name of myDisplayNames) { if (gpText === name) { gpMatch = true; break; } }
            if (gpMatch && !gp.hasAttribute("data-cp-me")) gp.setAttribute("data-cp-me", "1");
        }
    }
    applyInlineFontStyles(false);
}
    ...
    try {
        ...
        for (const loc of locales) {
            for (const fmt of fmtSpecs) {
                try { const s = new Intl.DateTimeFormat(loc, fmt).format(d); variants.add(s); variants.add(s.replace(/\s/g," ")); variants.add(s.replace(/\s/g,"\u00a0")); } catch { }
            }
        }
        ...
        for (const p of [`${day} ${mS} ${year}`,`${day} ${mL} ${year}`,`${mS} ${day}, ${year}`,`${mL} ${day}, ${year}`,d.toISOString().slice(0,10)]) {
            variants.add(p); variants.add(p.replace(/ /g,"\u00a0")); variants.add(p.replace(/\u00a0/g," "));
        }
    ...
    if (storedData.createdAt) {
        ...
        if (realDates.length > 0 && fakeDates.length > 0) {
            for (const rd of realDates) {
                if (rd.length >= 4 && (val.includes(rd) || val.toLowerCase().includes(rd.toLowerCase()))) {
                    result = result.split(rd).join(fakeDates[0]); replaced = true;
                }
            }
        }
    }
...
async function copyUserProfile(userId: string) {
    try {
        ...
        const newData: CustomProfileData = {
            ...
            banner: "", avatar: "", badgeFlags: 0, customBadgeIds: [],
            nitro: false, nitroLevel: -1, boostMonths: -1, giftingLevel: -1,
...
function NameplateBtn({ np, active, onClick }: { np: typeof NAMEPLATES[0]; active: boolean; onClick: () => void; }) {
    ...
    return (
        <button onClick={onClick} title={np.label} style={{
            padding:"0 10px", width:"100%", height:56, borderRadius:8, overflow:"hidden",
            display:"flex", alignItems:"center", justifyContent:imgSrc?"center":"flex-start", gap:8,
            position:"relative", border:active?"2px solid #5865f2":"2px solid var(--background-modifier-accent)",
            boxShadow:active?"0 0 0 2px #5865f2":"none",
            background:active?"rgba(88,101,242,0.18)":"var(--background-secondary)",
            flexShrink:0, cursor:"pointer", transition:"border-color 0.1s, box-shadow 0.1s, background 0.1s",
        }}>
            {imgSrc ? (
                <img src={imgSrc} alt={np.label} style={{ width:"100%",height:"100%",objectFit:"cover",objectPosition:"center",display:"block",position:"absolute",inset:0 }} />
            ) : (
                <>
                    {emoji && <span style={{ fontSize:22, lineHeight:1, flexShrink:0 }}>{emoji}</span>}
                    <span style={{ fontSize:11, fontWeight:700, color:"var(--header-primary)", textAlign:"left", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", maxWidth:"100%", lineHeight:1.3 }}>{np.label}</span>
                </>
            )}
        </button>
    );
...
function BadgePicker({ selected, onChange, nitroType, onNitroType, boostLevel, onBoostLevel, customIds, onCustomIds, oldName, onOldName, giftingLevel, onGiftingLevel }: {
    ...
    giftingLevel: number; onGiftingLevel: (v: number) => void;
}) {
    ...
    return (
        <div className="cp-field">
            ...
            <div className="cp-badges">
                ...
                {NITRO_LEVELS.map((n, i) => <BadgeBtn key={i} label={n.label} icon={n.icon} active={nitroType === i} onClick={() => onNitroType(i)} />)}
            </div>
            <SectionLabel style={{ marginTop:8 }}>{t("Gifting Badge")}</SectionLabel>
            <div style={{ fontSize:11, color:"var(--text-muted)", marginBottom:6 }}>{t("Evolving badge earned by gifting Nitro to others.")}</div>
            <div className="cp-badges">
                <BadgeBtn label={t("None")} active={giftingLevel === -1} onClick={() => onGiftingLevel(-1)} />
                {GIFTING_BADGE_LABELS.map((lbl, i) => (
                    <BadgeBtn key={i} label={lbl} icon={GIFTING_BADGE_ICONS[i]} active={giftingLevel === i} onClick={() => onGiftingLevel(i)} />
                ))}
            </div>
...
function FontPicker({ fontId, effect, color, color2, onFontId, onEffect, onColor, onColor2, displayName }: {
    ...
    onColor: (v: string) => void; onColor2: (v: string) => void;
    displayName: string;
}) {
    ...
    const previewText   = React.useMemo(() => {
        ...
        try { const u = UserStore.getCurrentUser(); if (u?.globalName?.trim()) return u.globalName.trim(); if (u?.username?.trim()) return u.username.trim(); } catch { }
        return "Display Name";
    }, [displayName]);

    const previewStyle: React.CSSProperties = {
        fontFamily: previewFont.family, fontWeight: previewFont.weight,
        fontStyle: previewFont.italic ? "italic" : "normal",
        fontSize: localFontId === 6 ? 13 : 22, lineHeight: 1.3,
        transition: "all 0.15s", display: "inline-block",
    };
    switch (localEffect) {
        case "neon":
            previewStyle.color = baseColor; (previewStyle as any).WebkitTextFillColor = baseColor;
            (previewStyle as any).filter = `drop-shadow(0 0 2px ${baseColor}) drop-shadow(0 0 6px ${baseColor}) drop-shadow(0 0 12px ${baseColor})`;
            break;
        case "gradient":
            previewStyle.background = `linear-gradient(90deg,${baseColor},${secondColor})`;
            previewStyle.WebkitBackgroundClip = "text"; (previewStyle as any).WebkitTextFillColor = "transparent";
            previewStyle.backgroundClip = "text"; previewStyle.color = "transparent";
            break;
        case "toon":
            previewStyle.color = baseColor; (previewStyle as any).WebkitTextFillColor = baseColor;
            (previewStyle as any).WebkitTextStroke = "2px #000000"; (previewStyle as any).paintOrder = "stroke fill";
            break;
        case "pop":
            previewStyle.color = baseColor; (previewStyle as any).WebkitTextFillColor = baseColor;
            ...
            break;
        default:
            previewStyle.color = localColor || "var(--header-primary)"; break;
    }
    ...
    return (
        <div className="cp-field">
            ...
            <div style={{ marginBottom:6, fontSize:11, color:"var(--text-muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>{t("Choose Font")}</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6, marginBottom:14 }}>
                {DISPLAY_NAME_FONTS.map(font => (
                    <button key={font.fontId} onClick={() => handleFontId(font.fontId)} title={font.label}
                        className={`cp-badge ${localFontId===font.fontId?"cp-badge--on":""}`}
                        style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"10px 4px 6px", height:68, borderRadius:8, gap:4 }}>
                        <span style={{ fontFamily:font.family, fontSize:font.fontId===6?12:22, lineHeight:1.2, fontWeight:font.weight, fontStyle:font.italic?"italic":"normal", color:"var(--header-primary)" }}>{font.sample}</span>
                        <span style={{ fontSize:9, opacity:0.65, letterSpacing:0.2, textTransform:"uppercase" }}>{font.label}</span>
                    </button>
                ))}
            </div>
            <div style={{ marginBottom:6, fontSize:11, color:"var(--text-muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>{t("Choose Effect")}</div>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
                {DISPLAY_NAME_EFFECTS.map(ef => (
                    <button key={ef.id} onClick={() => handleEffect(ef.id)} className={`cp-badge ${localEffect===ef.id?"cp-badge--on":""}`} style={{ padding:"6px 14px", borderRadius:8, fontSize:13, fontWeight:600 }}>{ef.label}</button>
                ))}
            </div>
            <div style={{ marginBottom:6, fontSize:11, color:"var(--text-muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>{isGradient?t("Gradient Colours"):t("Choose Colour")}</div>
            {isGradient ? (
                <div style={{ marginBottom:14 }}>
                    <div style={{ display:"flex", gap:16, alignItems:"flex-start", marginBottom:10 }}>
                        <div style={{ flex:1 }}>
                            ...
                            <div style={{ display:"flex", gap:4, alignItems:"center", flexWrap:"wrap" }}>
                                <input type="color" value={localColor||"#00c8ff"} onChange={e=>handleColor(e.target.value)} style={{ width:36,height:36,borderRadius:6,border:"2px solid var(--background-modifier-accent)",padding:2,cursor:"pointer",background:"none" }} />
                                <button onClick={()=>handleColor("")} className={`cp-badge ${!localColor?"cp-badge--on":""}`} style={{ width:32,height:32,borderRadius:6,padding:0,display:"flex",alignItems:"center",justifyContent:"center" }} title={t("Default")}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg></button>
                            </div>
                        </div>
                        <div style={{ display:"flex", alignItems:"center", height:36, marginTop:20, color:"var(--text-muted)", fontSize:18 }}>→</div>
                        <div style={{ flex:1 }}>
                            <div style={{ fontSize:10, color:"var(--text-muted)", marginBottom:4, fontWeight:600 }}>{t("Colour 2")}</div>
                            <div style={{ display:"flex", gap:4, alignItems:"center", flexWrap:"wrap" }}>
                                <input type="color" value={localColor2||"#ff40ff"} onChange={e=>handleColor2(e.target.value)} style={{ width:36,height:36,borderRadius:6,border:"2px solid var(--background-modifier-accent)",padding:2,cursor:"pointer",background:"none" }} />
                                <button onClick={()=>handleColor2("")} className={`cp-badge ${!localColor2?"cp-badge--on":""}`} style={{ width:32,height:32,borderRadius:6,padding:0,display:"flex",alignItems:"center",justifyContent:"center" }} title={t("Default")}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                        {GRADIENT_PRESETS.map((gp,i) => (
                            <button key={i} onClick={()=>{handleColor(gp.from);handleColor2(gp.to);}}
                                style={{ width:36,height:36,borderRadius:6,padding:0,cursor:"pointer",flexShrink:0, background:`linear-gradient(135deg,${gp.from},${gp.to})`, border:(localColor===gp.from&&localColor2===gp.to)?"2px solid #5865f2":"2px solid transparent", outline:(localColor===gp.from&&localColor2===gp.to)?"1px solid #5865f2":"none" }}
                                title={`${gp.from} → ${gp.to}`}
                            />
                        ))}
                    </div>
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center", marginBottom:14 }}>
                    <button onClick={()=>handleColor("")} className={`cp-badge ${!localColor?"cp-badge--on":""}`} style={{ width:32,height:32,borderRadius:6,padding:0,display:"flex",alignItems:"center",justifyContent:"center" }} title={t("Default")}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg></button>
                    <input type="color" value={localColor||"#ffffff"} onChange={e=>handleColor(e.target.value)} style={{ width:32,height:32,borderRadius:6,border:"2px solid var(--background-modifier-accent)",padding:2,cursor:"pointer",background:"none" }} title={t("Custom colour")} />
                    {DISPLAY_NAME_SOLID_COLORS.map(hex => (
                        <button key={hex} onClick={()=>handleColor(hex)}
                            style={{ width:28,height:28,borderRadius:6,background:hex,border:localColor===hex?"2px solid #5865f2":"2px solid transparent",outline:localColor===hex?"1px solid #5865f2":"none",cursor:"pointer",padding:0,flexShrink:0 }}
                            title={hex}
                        />
                    ))}
                </div>
...
function CustomProfileModal({ rootProps }: { rootProps: any }) {
    ...
    const giftingLevel = data.giftingLevel ?? -1;
        ...
        <ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="cp-content">
                ...
                <BadgePicker
                    ...
                    giftingLevel={giftingLevel} onGiftingLevel={v => set("giftingLevel", v)}
                />
                <div className="cp-divider"/>
                <FontPicker
                    key={saveCount}
                    fontId={data.displayNameFont ?? 0} effect={data.displayNameEffect ?? "solid"}
                    color={data.displayNameColor ?? ""} color2={data.displayNameColor2 ?? ""}
                    onFontId={v => set("displayNameFont", v)} onEffect={v => set("displayNameEffect", v)}
                    onColor={v => set("displayNameColor", v)} onColor2={v => set("displayNameColor2", v)}
                    displayName={previewDisplayName}
                />
                <div className="cp-divider"/>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}><SectionLabel>{t("Avatar decoration")}</SectionLabel></div>
                <div className="cp-badges" style={{ flexWrap:"wrap", gap:6 }}>
                    ...
                    {AVATAR_DECORATIONS.map(dec => (
                        <button key={dec.id} onClick={() => set("decorationAsset", data.decorationAsset === dec.asset ? undefined : dec.asset)}
                ...
                <div className="cp-field" style={{ marginTop:8 }}>
                    ...
                    <input className="cp-input" placeholder={t("Paste a decoration asset (e.g. a_xxxxxxxx)...")}
                ...
                <div className="cp-divider"/>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}><SectionLabel>{t("Profile Effect")}</SectionLabel></div>
                <div className="cp-hints" style={{ fontSize:11, color:"var(--text-muted)", marginBottom:8 }}>{t("Animated effect displayed on your profile page.")}</div>
                <div className="cp-badges" style={{ flexWrap:"wrap", gap:6 }}>
                    <button onClick={() => set("profileEffectId", undefined)} className={`cp-badge ${!data.profileEffectId?"cp-badge--on":""}`} style={{ minWidth:60 }}>{t("None")}</button>
                    {PROFILE_EFFECTS.map(effect => (
                        <button key={effect.id} onClick={() => set("profileEffectId", data.profileEffectId === effect.id ? undefined : effect.id)}
                            className={`cp-badge ${data.profileEffectId === effect.id ? "cp-badge--on" : ""}`} title={effect.label}
                            style={{ padding:3, lineHeight:0, width:52, height:52, borderRadius:6 }}>
                            <img src={effect.thumb} alt={effect.label} style={{ width:46, height:46, objectFit:"contain", display:"block" }}
                                onError={e => { (e.target as HTMLImageElement).style.display="none"; }} />
                        </button>
                    ))}
                </div>
                <div className="cp-field" style={{ marginTop:8 }}>
                    ...
                    <input className="cp-input" placeholder={t("Paste a collectibles-shop asset ID...")}
                ...
                <div className="cp-divider"/>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}><SectionLabel>{t("Nameplate")}</SectionLabel></div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8, marginTop:8, marginBottom:8 }}>
                    <button onClick={() => set("nameplateSkuId", undefined)}
                        style={{ gridColumn:"1 / -1", height:36, borderRadius:8, fontWeight:600, fontSize:13,
                            ...
                            color:"var(--header-primary)", cursor:"pointer", transition:"border-color 0.1s, box-shadow 0.1s, background 0.1s" }}>
                        {t("None")}
                    </button>
                    {NAMEPLATES.map(np => (
                        <NameplateBtn key={np.skuId} np={np} active={data.nameplateSkuId === np.skuId}
                ...
                <div className="cp-field" style={{ marginTop:8 }}>
                    ...
                    <input className="cp-input" placeholder={t("Paste a nameplate SKU ID...")}
...
export default definePlugin({
    ...
    patches: [
        {
            find: ':"SHOULD_LOAD");',
            replacement: { match: /\i(?:\?)?.getPreviewBanner\(\i,\i,\i\)(?=.{0,100}"COMPLETE")/, replace: "$self.patchBannerUrl(arguments[0])||$&" }
        },
        {
            find: ".WIDGETS_RTC_UPSELL_COACHMARK)",
            replacement: { match: /currentUser:(\i)(?=.{0,200}voiceDb)/, replace: "currentUser:$self.fakeCurrentUser($1)" }
        },
        {
            find: "DISPLAY_NAME", noWarn: true,
            replacement: { match: /(?<=currentUser:\i,user:)(\i)/, replace: "$self.fakeCurrentUser($1)" }
        },
        {
            find: "obfuscatedEmail", noWarn: true,
            replacement: [
        ...
        },
        {
            ...
            replacement: [{ noWarn:true, match: /user:([A-Za-z_$][\w$]*),displayProfile:([A-Za-z_$][\w$]*),themeType/, replace: "user:$self.fakeCurrentUser($1),displayProfile:$2,themeType" }]
        },
        {
            find: "AccountPanel",
        ...
        },
        {
            ...
            replacement: [
                { match: /obfuscatedEmail:([^,}]+)/g, replace: "obfuscatedEmail:$self.fakeObfuscatedEmail($1)" },
                { match: /obfuscatedPhone:([^,}]+)/g, replace: "obfuscatedPhone:$self.fakeObfuscatedPhone($1)" },
            ]
        },
    ],
    ...
    fakeCurrentUser(user: any) {
        ...
        if (!isEnabled) {
            clone.username = realUsername; clone.globalName = realGlobalName; clone.displayName = realDisplayName;
        ...
        if (isEnabled && storedData.nitro) {
            ...
            const since = new Date(); since.setMonth(since.getMonth() - (LEVEL_MONTHS[storedData.nitroLevel!] ?? 1));
            clone.premiumSince = since;
            const bm = storedData.boostMonths ?? -1;
            if (bm >= 0) {
                const BOOST_M = [1,2,3,6,9,12,15,18,24];
                const boostSince = new Date(); boostSince.setMonth(boostSince.getMonth() - (BOOST_M[bm] ?? 1));
    ...
    hookUserProfile(profile: any) {
        ...
        try {
            ...
            if (storedData.profileEffectId) {
                const effect = PROFILE_EFFECTS.find(e => e.id === storedData.profileEffectId);
                merged.profileEffect = { id: storedData.profileEffectId, skuId: storedData.profileEffectId, effectDefinitionId: effect?.effectDefinitionId ?? storedData.profileEffectId };
            }
            if (storedData.displayNameFont != null && storedData.displayNameFont > 0) { merged.displayNameFont = storedData.displayNameFont; merged.premiumType = 2; }
            if (storedData.displayNameColor) {
            ...
            if (isEnabled && (storedData.nitro || storedData.badgeFlags != null)) {
                ...
                if (storedData.nitro) {
                    if (storedData.accentColor != null) { const c2 = storedData.accentColor2 ?? storedData.accentColor; merged.themeColors = [storedData.accentColor, c2]; }
                    const nl = storedData.nitroLevel ?? 0;
                    const LEVEL_MONTHS = [1,2,3,6,12,24,36,72];
                    const since = new Date(); since.setMonth(since.getMonth() - (LEVEL_MONTHS[nl] ?? 1));
    ...
    fakeObfuscatedPhone(real: string | null) {
        ...
        const fake = storedData.phone; if (fake.length < 4) return fake;
        return "***-***-" + fake.slice(-4);
    },
    ...
    async start() {
        ...
        try {
            ...
            console.error("[CustomProfile] start() fatal error:", err);
            if (!loadError) showLoadNotification(false);
            removeHideStyle();
        }
    },
    ...
    userProfileBadges: [
        {
            getBadges({ userId, badges: nativeBadges }: { userId: string; guildId: string; badges: ProfileBadge[] }) {
                ...
                const style = { borderRadius: "0%", width: "22px", height: "22px" };
                const nl = storedData.nitroLevel  ?? -1;
                const bm = storedData.boostMonths ?? -1;
                const gl = storedData.giftingLevel ?? -1;
                const hasNitroFake   = nl >= 0 && nl < NITRO_LEVELS.length;
                const hasBoostFake   = bm >= 0 && bm < BOOST_ICONS.length;
                const hasGiftingFake = gl >= 0 && gl < GIFTING_BADGE_ICONS.length;
                ...
                badges = badges.filter(b => {
                    ...
                    if (hasGiftingFake && ["gifting","gift","patron","champion","luminary","icon","hero","legend"].some(k => desc.includes(k))) return false;
                    for (const badge of BADGES) {
                        if (wantedFlags & badge.flag) {
                            const kw = badge.label.toLowerCase().split(" ");
                            if (kw.some(k => k.length > 3 && desc.includes(k))) return false;
                ...
                if (hasGiftingFake)                      badgeList.push({ description: GIFTING_BADGE_LABELS[gl],   iconSrc: GIFTING_BADGE_ICONS[gl], props:{ style, title: GIFTING_BADGE_LABELS[gl] } });
