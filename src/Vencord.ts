...
import "~plugins";
console.log("%c[Fancord]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("fancord-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "fancord-core-updater-root";
    ...
    titleSpan.textContent = "🔔 Fancord Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Fancord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Fancord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Fancord.",
