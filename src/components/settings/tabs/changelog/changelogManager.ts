...
const FANCORD_RELEASES_REPO = "saintmanuser/fancord";
const FANCORD_REPO_URL = `https://github.com/${FANCORD_RELEASES_REPO}`;
...
export { FANCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Fancord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(FANCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
