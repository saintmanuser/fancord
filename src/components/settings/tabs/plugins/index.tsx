...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Fancord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.FANCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("fancord-tutorial-detected", handler);
        return () => window.removeEventListener("fancord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.FANCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/fancordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/fancordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { fancordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const fancordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Fancord to function."
                ...
                const isFancord = folderName.startsWith("src/fancordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isFancord) {
                    fancordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { fancordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...fancordPlugins, ...othersPlugins];
    ...
    const fancordVisible = fancordPlugins.slice(0, Math.min(visibleCount, fancordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - fancordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Fancord Plugins", value: SearchStatus.FANCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {fancordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...fancordVisible, ...othersVisible].length
                                    ? [...fancordVisible, ...othersVisible]
                ...
                <>
                    {fancordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Fancord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {fancordVisible}
                            </div>
                        </>
                    ...
                    {fancordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
