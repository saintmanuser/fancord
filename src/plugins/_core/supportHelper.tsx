...
import { platformName } from "@fancordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Fancord developers"
        const fancordGroupIds = [
        ...
        const isFancordGroup = fancordGroupIds.includes(userId);

        if (!isOriginalDev && !isFancordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Fancord developers";
