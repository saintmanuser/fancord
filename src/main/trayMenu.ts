...
function createFancordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Fancord",
            submenu: [
                ...
                {
                    label: cachedUpdateAvailable ? "Update Fancord" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Fancord",
...
export function patchTrayMenu(): void {
    // On ne patch plus le menu tray — Fancord gère son propre tray dans patcher.ts
}
