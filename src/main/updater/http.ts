...
const ZIP_FILE = "fancord-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `fancord-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `fancord-extract-${Date.now()}`);
...
app.on("before-quit", event => {
    ...
    console.log("[Fancord] Applying pending update before quit...");

    const safetyTimeout = setTimeout(() => {
        console.error("[Fancord] Update on quit timed out — forcing exit.");
    ...
    applyUpdates()
        .then(ok => {
            if (ok) console.log("[Fancord] Update applied successfully on quit.");
            else console.warn("[Fancord] Update on quit returned false.");
        })
        .catch(err => {
            console.error("[Fancord] Update on quit failed:", err);
