...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Fancord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Fancord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Fancord");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[Fancord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[Fancord] Running in vanilla mode. Not loading Fancord");
}

console.log("[Fancord] Loading original Discord app.asar");
