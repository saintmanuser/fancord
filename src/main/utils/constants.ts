...
export const DATA_DIR = process.env.FANCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "FancordData", suffix)
        : join(app.getPath("userData"), "..", "Fancord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Fancord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
