...
// ── CRITIQUE : userData = dossier Fancord pour les settings/plugins
...
const fancordData = path.join(app.getPath("appData"), "Fancord");
app.setPath("userData", fancordData);
...
// pour le token Discord stable. Quand Fancord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Fancord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Fancord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Fancord] Module bloqué (inutile pour Fancord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Fancord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Fancord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Fancord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(fancordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Fancord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Fancord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Fancord] LevelDB check failed:", e.message); }

// Modules bundlés dans fancord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Fancord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Fancord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans fancord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Fancord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Fancord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Fancord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Fancord] Impossible de patcher build_info.json:", e.message);
}
