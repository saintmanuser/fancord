@echo off
:: ─── Fancord — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=Fancord %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\fancord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Fancord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    FANCORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler Fancord-Installer.exe ──────────────────────────────────────
echo.
echo  [5/8] Compilation de Fancord-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Fancord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Fancord-Installer.exe cree (%%~zF octets)

:: ── 6. Créer fancord-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de fancord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'fancord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer fancord-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] fancord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/fancordoff/fancord/releases/latest/download/Fancord-Installer.exe",
    echo   "distUrl": "https://github.com/fancordoff/fancord/releases/latest/download/fancord-dist.zip",
    echo   "downloadUrl": "https://github.com/fancordoff/fancord/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#Fancord-Installer.exe" ^
    "%DIST_ZIP%#fancord-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo fancordoff/fancord ^
    --title "Fancord v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  Fancord v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    Fancord-Installer.exe    — installeur .exe avec GUI
echo  ║    fancord-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront Fancord-Installer.exe
