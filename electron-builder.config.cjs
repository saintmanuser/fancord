...
// ─── Configuration de Build Fancord ─────────────────────────────────────────

function killFancord() {
    const releaseDir = join(__dirname, "release", "fancord-dist");
...
function buildFancordFromDiscord(discordApp) {
    ...
    console.log("[fancord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Fancord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "fancord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const fancordPreload = join(__dirname, "fancord-preload.js");
    if (existsSync(fancordPreload)) {
        cpSync(fancordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "fancord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Fancord" --set-version-string "FileDescription" "Fancord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[fancord] Build terminé -> ${outDir}`);
}
...
buildFancordFromDiscord(discord);

module.exports = {
    appId: "com.fancord.app",
    productName: "Fancord",
    copyright: "Copyright 2026 Fancord",
