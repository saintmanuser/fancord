...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "fancordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/fancord/renderer.js",
        format: "iife",
        target: ["esnext"],
    ...
    },
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/preload.ts")],
        outfile: "dist/fancord/preload.js",
        footer: { js: "//# sourceURL=file:///VencordPreload\n" + sourceMapFooter("preload") },
        sourcemap,
        define: {
            ...defines,
...
await Promise.all([
    ...
    writeFile("dist/fancord/package.json", JSON.stringify({
        name: "fancord",
        main: "main.js"
    }))
]);
...
if (!watch && !IS_DEV) {
    await Promise.all([
        ...
        obfuscateDir("dist/fancord"),
    ]);
} else {
    console.log("[build] Obfuscation skipped in dev/watch mode.");
}
*/
console.log("[build] Obfuscation disabled (source code in plain text).");

await Promise.all([
    createPackage("dist/desktop", "dist/desktop.asar"),
    createPackage("dist/fancord", "dist/fancord.asar"),
]);
