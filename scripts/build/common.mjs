...
export const gitHash = process.env.FANCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "fancordplugins", "fancordplugins/_api"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.FANCORD_REMOTE;
            if (!remote) {
                try {
...
export const commonOpts = {
    ...
    alias: {
        ...
        "@fancordplugins": "./src/fancordplugins",
    }
};
