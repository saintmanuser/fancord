# ==============================================================================
#  Fancord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Fancord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Fancord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          FANCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Fancord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Fancord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Fancord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$FancordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Fancord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "fancord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'fancord-dist.zip' introuvable dans la release $version.`n           Contactez le support Fancord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "fancord-dist.zip"
    ...
    Write-OK "Fancord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Fancord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Fancord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Fancord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Fancord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez fancord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
